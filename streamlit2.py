...
DATABASE_URL = r"mysql+pymysql://4DqqoiZXMnHx5MN.root:3wJPqKRiThuBOBDz@gateway01.ap-southeast-1.prod.aws.tidbcloud.com:4000/Stock?ssl_ca=C:\Users\vishn\OneDrive\Desktop\project2\isrgrootx1 .pem"
...
if not volatility_df.empty:
    ...
    st.markdown(">  **Insight:** These stocks have the highest price fluctuations, indicating high risk and potential high reward.")
...
if not cum_df.empty:
    ...
    st.markdown( ">  **Insight:** These top 5 stocks have consistently outperformed others over time, indicating strong growth trends.")
...
if not sector_df.empty:
    fig_sector = px.bar(sector_df,x='sector', y='cumulative_return', title="Cumulative Return by Sector", color='sector')
    st.plotly_chart(fig_sector, use_container_width=True) 
    st.markdown(">  **Insight:** Sectors with the highest average returns can guide long-term investment decisions.")
...
if not corr_df.empty and {"ticker1", "ticker2","correlation"}.issubset(corr_df.columns): 
    ...
    fig_corr = px.imshow(pivot_corr,labels=dict(color="correlation"), x=pivot_corr.columns, y=pivot_corr.index, title="Stock Price Correlation Heatmap", color_continuous_scale="RdBu",zmin=-1, zmax=1)

    fig_corr.update_layout(width=1000, height=1000, xaxis=dict(tickangle=45), margin=dict(l=50, r=50, t=80, b=100), font=dict(size=12) )
    st.plotly_chart(fig_corr, use_container_width=True)
    st.markdown(">  **Insight:** Strongly correlated stocks often move together, which helps in risk diversification strategies.")
else:
...
if not monthly_df.empty:
    ...
    with col1:
        ...
        st.markdown(">  **Insight:** These stocks saw the highest monthly returns, possibly due to strong earnings, news, or sector momentum.")

    with col2:
        st.subheader(f" Top 5 Losers - {selected_month}")
        fig_losers = px.bar(losers, x="ticker", y="monthly_return", color="ticker")
        st.plotly_chart(fig_losers, use_container_width=True, key="losers_chart_1")
        

        st.markdown(">  **Insight:** These underperforming stocks may indicate sector weakness or negative sentiment.")
